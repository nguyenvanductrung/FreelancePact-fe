...
import {
  ...
  TrendingUp,
  ChevronRight,
  XSquare, // <-- Thêm icon này để làm nút Từ chối nghiệm thu (nếu cần)
} from "lucide-react";
...
interface ContractsTabProps {
  onTriggerReject: (id: string, title: string) => void;
  onTriggerSubmit: (id: string, title: string) => void; // Thêm trigger nộp bài
}
function ContractsTab({ onTriggerReject, onTriggerSubmit }: ContractsTabProps) {
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-8 text-center text-gray-400 shadow-sm flex flex-col items-center">
      <Briefcase className="w-8 h-8 mx-auto mb-2 opacity-40" />
      <p className="text-sm">Danh sách hợp đồng của Elena sẽ hiển thị tại đây.</p>
      
      <div className="flex gap-3 mt-4">
        {/* Nút trigger mở Modal Từ chối nghiệm thu */}
        <button 
          onClick={() => onTriggerReject("MS-99", "Hoàn thành Wireframe Trang chủ")}
          className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-bold text-red-600 bg-red-50 border border-red-200 rounded-xl hover:bg-red-100 transition-all active:scale-95"
        >
          Từ chối nghiệm thu (Demo)
        </button>

        {/* Nút trigger mở Modal Nộp nghiệm thu mới thêm */}
        <button 
          onClick={() => onTriggerSubmit("MS-99", "Hoàn thành Wireframe Trang chủ")}
          className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-bold text-emerald-600 bg-emerald-50 border border-emerald-200 rounded-xl hover:bg-emerald-100 transition-all active:scale-95"
        >
          Nộp bài nghiệm thu (Demo)
        </button>
      </div>

      <Link href="/contracts/CTR-2024-892" className="inline-flex items-center gap-1 mt-4 text-sm font-semibold" style={{ color: BLUE }}>
...
function SettingsTab() { return ( <div className="bg-white border border-gray-200 rounded-2xl p-8 text-center text-gray-400">Cài đặt tài khoản</div> ); }
...
export default function ProfilePage() {
  ...
  const handleOpenRejectModal = (id: string, title: string) => {
    setSelectedMilestone({ id, title });
    setIsRejectModalOpen(true);
  };

  const handleOpenSubmitModal = (id: string, title: string) => {
    setSelectedMilestone({ id, title });
    setIsSubmitModalOpen(true);
  };

  const handleRejectSuccess = () => {
    console.log(`Từ chối thành công milestone ${selectedMilestone?.id}. Cập nhật lại UI phòng chat...`);
    // Thêm hàm fetch lại dữ liệu mới nếu cần thiết tại đây
  };
    ...
    >
      ...
      <main className="flex-1 flex gap-6 px-6 py-6 max-w-[1200px] mx-auto w-full">
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          ...
          {/* Truyền callback function kích hoạt mở modal vào Tab hợp đồng */}
          {activeTab === "contracts" && (
            <ContractsTab 
              onTriggerReject={handleOpenRejectModal} 
              onTriggerSubmit={handleOpenSubmitModal} 
            />
          )}
      ...
      {selectedMilestone && isSubmitModalOpen && (
        <SubmitMilestoneModal
          ...
          milestoneTitle={selectedMilestone.title}
          onSubmitSuccess={() => {
              console.log("Nộp minh chứng nghiệm thu thành công!");
          }}
        />
      )}
